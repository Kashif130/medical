...
const COL_MAP = {
  name:         ["name", "medicine", "medicine name", "naam", "دوائی"],
  category:     ["category", "type", "qism"],
  batchNo:      ["batch", "batch no", "batch number", "batchno"],
  stock:        ["stock", "quantity", "qty", "miqdar"],
  reorderLevel: ["reorder", "reorder level", "min stock"],
  costPrice:    ["cost", "cost price", "purchase price", "lागत"],
  price:        ["price", "sale price", "selling price", "qeemat"],
...
function excelDateToString(val) {
  ...
  // Already a string like "2026-12-31" or "31/12/2026"
...
export default function InventoryPage() {
  ...
  function downloadTemplate() {
    const headers = ["Name","Category","Pack Size","Manufacturer","Batch No","Stock","Reorder Level","Cost Price","Price","Expiry Date"];
    const sample  = ["Panadol 500mg","Tablet","100S","GSK","BT2024A","200","20","8","12","2027-06-30"];
    ...
    reader.onload = (ev) => {
      try {
        ...
        const rows = dataRows.map((row, idx) => {
          ...
          if (!mapped.name) errors.push(`Row ${idx + 2}: Name column missing ya khali hai`);
          if (!mapped.price) errors.push(`Row ${idx + 2}: Price column missing ya khali hai`);
          return {
            ...mapped,
            stock: Number(mapped.stock || 0),
            reorderLevel: Number(mapped.reorderLevel || 10),
            price: Number(mapped.price || 0),
            costPrice: Number(mapped.costPrice || 0),
            expiryDate: excelDateToString(mapped.expiryDate),
          };
        });
        setBulkErrors(errors);
        setBulkRows(rows);
      } catch (err) {
        setBulkErrors(["File read nahi ho saki: " + err.message]);
      }
    };
    ...
    <div style={{ padding: isMobile ? "16px" : "32px", maxWidth: 1100 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => { setShowBulk(true); setBulkDone(null); setBulkRows([]); setBulkErrors([]); }} style={{
            display: "flex", alignItems: "center", gap: 6, background: "white",
            color: "#0e6e5c", fontSize: 13, fontWeight: 500, padding: "8px 12px",
            borderRadius: 10, border: "1px solid #0e6e5c", cursor: "pointer"
          }}>
      ...
      <div style={{ position: "relative", marginBottom: 16 }}>
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Name, category, batch..."
              ...
              <div key={m.id} style={{ background: "white", border: "1px solid #dce6e2", borderLeft: `3px solid ${STATUS_COLOR[status]}`, borderRadius: 12, padding: "14px 14px 12px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ flex: 1, minWidth: 0, paddingRight: 8 }}>
                    ...
                    {(m.category || m.packSize) && <p style={{ fontSize: 11, color: "#9ca3af", margin: "2px 0 0" }}>{[m.category, m.packSize].filter(Boolean).join(" · ")}</p>}
                    {m.batchNo && <p style={{ fontSize: 11, color: "#9ca3af", margin: "1px 0 0", fontFamily: "monospace" }}>Batch: {m.batchNo}</p>}
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                    ...
                    <button onClick={() => openEdit(m)} style={{ background: "none", border: "none", cursor: "pointer", color: "#6b7280", padding: 2 }}><Pencil size={15} /></button>
                    {isAdmin && <button onClick={() => handleDelete(m.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#6b7280", padding: 2 }}><Trash2 size={15} /></button>}
                  </div>
                </div>
        ...
        <div style={{ background: "white", border: "1px solid #dce6e2", borderRadius: 12, overflow: "hidden" }}>
          <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse" }}>
            ...
            <tbody>
                ...
                return (
                  <tr key={m.id} style={{ borderBottom: "1px solid #f0f4f2", borderLeft: `3px solid ${STATUS_COLOR[status]}` }}>
                    <td style={{ padding: "10px 12px" }}>
                      ...
                      <p style={{ fontSize: 11, color: "#9ca3af", margin: "2px 0 0" }}>{[m.category, m.packSize, m.manufacturer].filter(Boolean).join(" · ")}</p>
                    </td>
              ...
              {filtered.length === 0 && <tr><td colSpan={9} style={{ padding: "40px", textAlign: "center", color: "#9ca3af", fontSize: 13 }}>Koi medicine nahi mili.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      ...
      {showForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 16 }}>
          <form onSubmit={handleSave} style={{ background: "white", borderRadius: 14, width: "100%", maxWidth: 480, padding: 24, maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <button type="submit" style={{ marginTop: 16, width: "100%", background: "#0e6e5c", color: "white", fontSize: 13, fontWeight: 500, padding: "10px", borderRadius: 10, border: "none", cursor: "pointer" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 16 }}>
          <div style={{ background: "white", borderRadius: 14, width: "100%", maxWidth: 560, padding: 24, maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
              <h2 style={{ fontSize: 15, fontWeight: 600, margin: 0 }}>Bulk upload — Excel</h2>
              <button onClick={() => setShowBulk(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "#9ca3af" }}><X size={18} /></button>
            </div>
            <p style={{ fontSize: 12, color: "#6b7280", marginBottom: 16 }}>
              Excel file se saari medicines ek baar mein upload karein.
            </p>

            {/* Step 1 — Download template */}
            <div style={{ background: "#f0faf5", border: "1px solid #a7f3d0", borderRadius: 10, padding: "14px 16px", marginBottom: 14 }}>
              <p style={{ fontSize: 13, fontWeight: 600, margin: "0 0 4px" }}>Step 1 — Template download karein</p>
              <p style={{ fontSize: 12, color: "#6b7280", margin: "0 0 10px" }}>
                Is template mein data fill karein. Columns ka naam mat badlein.
              </p>
              <button onClick={downloadTemplate} style={{ display: "flex", alignItems: "center", gap: 6, background: "#0e6e5c", color: "white", fontSize: 12, fontWeight: 500, padding: "7px 14px", borderRadius: 8, border: "none", cursor: "pointer" }}>
                <Download size={13} /> Template download karein (.xlsx)
              </button>
            </div>

            {/* Step 2 — Upload */}
            <div style={{ background: "#f8faf9", border: "2px dashed #dce6e2", borderRadius: 10, padding: "20px", textAlign: "center", marginBottom: 14, cursor: "pointer" }}
              ...
              <p style={{ fontSize: 12, color: "#9ca3af", margin: 0 }}>.xlsx ya .xls file</p>
            ...
            {bulkErrors.length > 0 && (
              <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, padding: "12px 14px", marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                  ...
                  <p style={{ fontSize: 13, fontWeight: 600, color: "#dc2626", margin: 0 }}>{bulkErrors.length} errors mili</p>
                </div>
            ...
            {bulkRows.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                <p style={{ fontSize: 13, fontWeight: 600, margin: "0 0 8px", color: "#0e6e5c" }}>
                  ✓ {bulkRows.length} medicines ready — preview:
                </p>
                <div style={{ maxHeight: 200, overflowY: "auto", border: "1px solid #dce6e2", borderRadius: 8 }}>
                  <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "#f8faf9" }}>
                        {["Name","Category","Stock","Cost","Price","Expiry"].map(h => (
                          <th key={h} style={{ padding: "6px 10px", textAlign: "left", fontFamily: "monospace", fontSize: 10, color: "#6b7280", textTransform: "uppercase", borderBottom: "1px solid #dce6e2" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {bulkRows.map((r, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid #f0f4f2" }}>
                          ...
                          <td style={{ padding: "5px 10px", fontFamily: "monospace" }}>Rs.{r.costPrice}</td>
                          <td style={{ padding: "5px 10px", fontFamily: "monospace" }}>Rs.{r.price}</td>
                          <td style={{ padding: "5px 10px", fontFamily: "monospace", fontSize: 11 }}>{r.expiryDate || "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {bulkDone && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#f0faf5", border: "1px solid #a7f3d0", borderRadius: 10, padding: "12px 14px", marginBottom: 14 }}>
                ...
                <p style={{ fontSize: 13, fontWeight: 500, margin: 0, color: "#16a34a" }}>
                  {bulkDone.success} medicines upload ho gayi!
                  {bulkDone.fail > 0 && ` (${bulkDone.fail} fail)`}
                </p>
              </div>
            )}
            ...
            {bulkRows.length > 0 && !bulkDone && (
              <button onClick={handleBulkUpload} disabled={bulkUploading || bulkErrors.length > 0}
                style={{ width: "100%", background: bulkErrors.length > 0 ? "#9ca3af" : "#0e6e5c", color: "white", fontSize: 13, fontWeight: 500, padding: "11px", borderRadius: 10, border: "none", cursor: bulkErrors.length > 0 ? "not-allowed" : "pointer" }}>
                {bulkUploading ? `Uploading ${bulkRows.length} medicines...` : `Upload ${bulkRows.length} medicines`}
              </button>
            )}
          </div>
        </div>
