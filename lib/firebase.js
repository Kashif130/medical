...
// Returns { "2026-07-14": costTotal, ... } keyed by local date string, derived from sales' costPrice*qty
export function watchSalesForCost(cb) {
  return onSnapshot(query(collection(db,"sales"), orderBy("createdAt","desc")), s => {
    const out = s.docs.map(d => {
      const data = d.data();
