...
export async function checkoutSale({ items, customerName, customerPhone, subtotal, flatDiscount, miscCharges, applyGst, total, paymentMethod, createdBy }) {
  return runTransaction(db, async (tx) => {
    ...
    tx.set(sRef, {
      ...
      // Return tracking fields — start at 0
...
export async function createReturn({ saleId, items, reason, refundAmount, refundMethod, createdBy }) {
  return runTransaction(db, async (tx) => {
    ...
    // 2. Sale record update karo — returnedAmount aur hasReturn
    if (saleId && !saleId.startsWith("OFFLINE")) {
      const saleRef  = doc(db,"sales",saleId);
      const saleSnap = await tx.get(saleRef);
      if (saleSnap.exists()) {
        const sd = saleSnap.data();
        const prevReturned     = sd.returnedAmount || 0;
        const newReturnedAmount = prevReturned + refundAmount;
        const prevItems        = sd.returnedItems || [];
        tx.update(saleRef, {
          returnedAmount: newReturnedAmount,
          // Net amount jo actually mili after returns
          netTotal: (sd.total || 0) - newReturnedAmount,
          hasReturn: true,
          returnedItems: [...prevItems, ...items.map(i=>({...i, refundedAt: new Date().toISOString()}))],
        });
      }
    }

    // 3. Return record banao
