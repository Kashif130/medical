...
// ── Practitioner daily earnings (manual entry, net after expenses split 70/30) ──
export const watchDailyEarnings = (cb) => onSnapshot(query(collection(db,"dailyEarnings"), orderBy("dateKey","desc")), s => cb(s.docs.map(d=>({id:d.id,...d.data()}))));

export async function upsertDailyEarning({ id, dateKey, amount, note, createdBy }) {
  ...
  if (id) {
    return updateDoc(doc(db,"dailyEarnings",id), data);
  }
  return addDoc(collection(db,"dailyEarnings"), { ...data, createdAt: serverTimestamp() });
}

export const deleteDailyEarning = (id) => deleteDoc(doc(db,"dailyEarnings",id));

// ── Daily expenses (kharcha, split 50/50 between partners) ──
export const watchDailyExpenses = (cb) => onSnapshot(query(collection(db,"dailyExpenses"), orderBy("dateKey","desc")), s => cb(s.docs.map(d=>({id:d.id,...d.data()}))));

export async function addDailyExpense({ dateKey, title, amount, createdBy }) {
  return addDoc(collection(db,"dailyExpenses"), {
    dateKey, title: title||"Kharcha",
...
export const updateDailyExpense = (id,d) => updateDoc(doc(db,"dailyExpenses",id), d);
export const deleteDailyExpense = (id)   => deleteDoc(doc(db,"dailyExpenses",id));

// ── Medicine cost lookup (for Daily Earnings profit calc) ──
// Combines sales cost (positive) and returns cost (negative, netted on the return's own date)
export function watchSalesForCost(cb) {
  let salesRows = [];
  let returnRows = [];
  ...
  function emit() {
    if (!salesReady || !returnsReady) return;
    cb([...salesRows, ...returnRows]);
  }

  const unsubSales = onSnapshot(query(collection(db,"sales"), orderBy("createdAt","desc")), s => {
    salesRows = s.docs.map(d => {
      const data = d.data();
      const dt = data.createdAt?.toDate ? data.createdAt.toDate() : null;
      const dateKey = dt ? dt.toISOString().slice(0,10) : null;
      const cost = (data.items||[]).reduce((sum,it)=> sum + (it.costPrice||0)*(it.qty||0), 0);
      return { id:d.id, dateKey, cost };
    });
    salesReady = true;
    emit();
  });

  const unsubReturns = onSnapshot(query(collection(db,"returns"), orderBy("createdAt","desc")), s => {
    returnRows = s.docs.map(d => {
      const data = d.data();
      const dt = data.createdAt?.toDate ? data.createdAt.toDate() : null;
      const dateKey = dt ? dt.toISOString().slice(0,10) : null;
      const cost = (data.items||[]).reduce((sum,it)=> sum + (it.costPrice||0)*(it.qty||0), 0);
      return { id:"ret-"+d.id, dateKey, cost: -cost }; // negative: nets out the original sale's cost
    });
    returnsReady = true;
    emit();
  });
